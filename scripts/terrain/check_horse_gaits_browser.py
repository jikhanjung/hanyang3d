...
try:
 ...
 with sync_playwright() as p:
  ...
  result=page.evaluate('''async()=>{const T=await import('three'),{createHorse}=await import('/webapp/static/horse_dealer.js');
   ...
   return {low,air,contacts:contacts.map(a=>[a[0],a.at(-1),a.length]),jumpStable};}''')
  print(result,flush=True);assert result['low']>-.005,result;assert result['air']>10,result;assert result['jumpStable'];assert all(c[2]>10 for c in result['contacts'])
  ...
  result=motion.evaluate('''async()=>{const T=await import('three'),{createFirstPerson}=await import('/webapp/static/first_person.js'),{createHorse}=await import('/webapp/static/horse_dealer.js');
   ...
   const horse=createHorse(),front=horse.group.getObjectByName('horse-front-left'),rear=horse.group.getObjectByName('horse-hind-right');let diagonal=true;
   const feet=l=>l.getObjectByName('horse-knee').localToWorld(new T.Vector3(0,-.425,0));
   for(let t=0;t<720;t+=30){horse.update(t,true,false,false);horse.group.updateMatrixWorld(true);const a=feet(front),b=feet(rear);diagonal&&=Math.abs(a.y-b.y)<1e-6&&Math.abs((a.z-front.position.z)-(b.z-rear.position.z))<1e-6;}
   return {trot,gallop,released,right,stopped,fast,slow,jumping,diagonal};}''')
