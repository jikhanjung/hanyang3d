"""Verify the remembered house sites: Song Siyeol's compound, Jeong Dojeon's stone marker and the settings toggle."""
import argparse
...
EXPECTED={'jeongdojeon_site':('정도전 집터',37.572539,126.979405,'site-marker'),'songsiyeol_site':('송시열 집터',37.58861,126.99689,'house-site')}
...
with sync_playwright() as p:
 ...
 # Song Siyeol keeps the walled compound, sized half as wide as it is deep.
 song=rows['songsiyeol_site']
 assert song['parts']>40 and song['meshes']<=8 and abs(song['size'][0]*2-song['size'][2])<=2,song
 ...
 # The Song Siyeol house faces south: its gate side lies south of its centre.
 facing=page.evaluate('''async()=>{const T=await import('/webapp/static/vendor/three/three.module.js'),t=terrain3d;
  t.scene.updateMatrixWorld(true);
  const b=t.buildings.children.find(b=>b.userData.feature.id==='songsiyeol_site'),d=b.userData.feature.symbol_size_m[2];
  const gate=b.localToWorld(new T.Vector3(0,0,d/2)),back=b.localToWorld(new T.Vector3(0,0,-d/2));
  return {gateSouthOfCentre:gate.z-b.position.z,backNorthOfCentre:b.position.z-back.z}}''')
 print(facing,flush=True)
 assert facing['gateSouthOfCentre']>0 and facing['backNorthOfCentre']>0,facing
