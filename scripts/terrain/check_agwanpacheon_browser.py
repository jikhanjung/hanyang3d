 ...
 with open(tmp+'/web.log','w') as log:
  ...
  try:
   ...
   with sync_playwright() as p:
    ...
    page=browser.new_page(viewport={'width':1280,'height':900});page.goto('http://127.0.0.1:18110/events/agwanpacheon/');page.wait_for_function('window.seoul1907?.ready',timeout=180000)
    page.evaluate("""async()=>{await seoul1907.walking.shop.whenReady;const csrf=document.cookie.split('; ').find(x=>x.startsWith('csrftoken='))?.slice(10);await fetch('/api/account/register',{method:'POST',headers:{'Content-Type':'application/json','X-CSRFToken':csrf},body:JSON.stringify({name:'가마검사',password:'test-secret'})});await seoul1907.walking.shop.refresh();await seoul1907.historicalEvent.begin()}""")
    ...
    result=page.evaluate("""async()=>{const s=seoul1907,e=s.historicalEvent;s.renderer.setAnimationLoop(null);for(let i=0;i<300;i++){e.regroup();e.update(.1)}await new Promise(r=>setTimeout(r,200));const reached=e.checkpoint;s.firstPerson.exit();e.update(.1);const paused=!e.active;await e.begin();const resumed=e.checkpoint===reached&&e.active;for(let i=0;i<10000&&e.active;i++){e.regroup();e.update(.1);if(i%50===0)await new Promise(r=>setTimeout(r,0))}await new Promise(r=>setTimeout(r,300));e.update(.1);await new Promise(r=>setTimeout(r,300));return {reached,paused,resumed,checkpoint:e.checkpoint,text:document.querySelector('#historical-event-panel').textContent,active:e.active}}""")
    print('COMPLETE',result,flush=True);assert result['reached']>=1 and result['paused'] and result['resumed'] and result['checkpoint']==7 and '회상 완료가 계정에 기록' in result['text'],result
