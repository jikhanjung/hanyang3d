 ...
 with open(tmp+'/web.log','w') as log:
  ...
  try:
   ...
   with sync_playwright() as p:
    ...
    result=page.evaluate("""()=>{const s=seoul1907,e=s.historicalEvent,m=e.module('procession');return {active:e.active,phase:e.phase,points:m.path.length,milestones:m.milestones,visible:s.buildings.filter(b=>b.visible).map(b=>b.userData.feature.id),connected:s.walking.together.connected,trams:s.trams.group.visible,chairCount:m.chairs.length,chairsShown:m.root.visible,contact:e.contacts.letter.p.group.visible,task:document.getElementById('historical-event-task').textContent}}""")
    ...
    assert page.evaluate("seoul1907.historicalEvent.module('procession').root.visible && !seoul1907.historicalEvent.contacts.gate.p.group.visible")
    ...
    check=page.evaluate("""()=>{const s=seoul1907,e=s.historicalEvent.module('procession'),C=globalThis.ChannelTerrain;let wet=0,blocked=0,crossing=0;for(let i=1;i<e.path.length;i++){const a=e.path[i-1],b=e.path[i],steps=Math.ceil(a.distanceTo(b)/.5);for(let j=0;j<=steps;j++){const p=a.clone().lerp(b,j/steps),m=C.nearest(p.x,p.z,s.channel.path),y=s.firstPerson.groundAt(p.x,p.z);if(m.distance<m.width){crossing++;if(y<m.level+.15)wet++}if(s.walking.collision.hit(p.x,p.z,1.35))blocked++}}return {houses:e.neighborhood.visibleCount,wet,blocked,crossing}}""")
    ...
    patrol=page.evaluate("""()=>{const s=seoul1907,e=s.historicalEvent,m=e.module('procession'),p=m.patrol,fp=s.firstPerson;s.renderer.setAnimationLoop(null);
     let i=0;for(;i<6000&&p.state==='pending';i++){e.regroup();e.update(.1)}
     const out={reached:p.state,after:i,soldiers:p.soldiers.length,visible:p.group.visible,spotOffRoad:Math.hypot(p.spot.x-m.chairs[0].group.position.x,p.spot.z-m.chairs[0].group.position.z)>4};
     const sp=p.soldiers[0].group.position;fp.placeAt(sp.x+20,sp.z,0);e.update(.1);out.seenFar=m.warnButton.hidden&&document.querySelector('#historical-event-panel p').textContent.includes('순찰');
     const lead=m.chairs[0].group.position;fp.placeAt(lead.x+4,lead.z+4,0);e.update(.1);out.button=!m.warnButton.hidden;m.warnButton.click();out.warned=p.state==='hiding'&&p.warned;
     let hid=0,maxOff=0;for(i=0;i<3000&&p.state!=='done';i++){e.update(.1);if(p.state==='hidden'){hid++;maxOff=Math.max(maxOff,Math.hypot(m.chairs[0].group.position.x-p.spot.x,m.chairs[0].group.position.z-p.spot.z));if(hid===20){const c=m.chairs[0].group.position,sp=p.soldiers[0].group.position,px=c.x+(sp.x-c.x)*.12,pz=c.z+(sp.z-c.z)*.12;fp.placeAt(px,pz,Math.atan2(-(c.x-px),-(c.z-pz)));e.update(.05);s.renderer.render(s.scene,s.camera);window.patrolShot=s.renderer.domElement.toDataURL()}}}
    ...
    result=page.evaluate("""async()=>{const s=seoul1907,e=s.historicalEvent,m=e.module('procession');s.renderer.setAnimationLoop(null);for(let i=0;i<300;i++){e.regroup();e.update(.1)}await new Promise(r=>setTimeout(r,200));const reached=e.checkpoint;s.firstPerson.exit();e.update(.1);const paused=!e.active;await e.begin();const resumed=e.checkpoint===reached&&e.active&&e.phase==='procession';for(let i=0;i<10000&&!(e.phase==='aftermath'&&m.arrival?.done);i++){if(!m.arrival)e.regroup();e.update(.1);if(!window.middleShot&&m.distance>700){s.renderer.render(s.scene,s.camera);window.middleShot=s.renderer.domElement.toDataURL()}if(!window.arrivalShot&&m.arrival?.time>6){s.renderer.render(s.scene,s.camera);window.arrivalShot=s.renderer.domElement.toDataURL();window.arrivalText=document.getElementById('royal-arrival-bubble').textContent}if(i%50===0)await new Promise(r=>setTimeout(r,0))}await new Promise(r=>setTimeout(r,300));e.update(.1);return {reached,paused,resumed,checkpoint:e.checkpoint,phase:e.phase,active:e.active}}""")
    ...
    # Leaving during the aftermath and coming back restores the finished procession silently and replays the talks.
    resumed=page.evaluate("""async()=>{const s=seoul1907,e=s.historicalEvent,d=s.walking.dialogue;d.end();s.firstPerson.exit();e.update(.1);const paused=!e.active;await e.begin();const m=e.module('procession');
     return {paused,phase:e.phase,checkpoint:e.checkpoint,arrivalDone:!!m.arrival?.done,hostShown:!!m.anchors().host,talking:!!d.current}}""")
    print('RESUME AFTERMATH',resumed,flush=True);assert resumed['paused'] and resumed['phase']=='aftermath' and resumed['checkpoint']==12 and resumed['arrivalDone'] and resumed['hostShown'] and resumed['talking'],resumed
    again=page.evaluate(AFTER,'');assert again['name']=='공사관의 통역',again
    result=page.evaluate(AFTER,'');print('COMPLETE',{k:v for k,v in result.items() if k!='text'},flush=True)
    assert page.evaluate("seoul1907.historicalEvent.lighting>0"),'dawn did not begin'
