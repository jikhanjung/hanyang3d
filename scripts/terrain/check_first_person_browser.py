...
with sync_playwright() as p:
 ...
 assert page.evaluate('Math.abs(Number(document.getElementById("walking-minimap").dataset.worldX)-terrain3d.firstPerson.eye.x)<1e-6')
 after=page.evaluate('terrain3d.camera.position.toArray()');distance=sum((after[i]-before[i])**2 for i in [0,2])**.5;assert 5.2<distance<6.1,distance
 box=page.locator('#scene > canvas').bounding_box();x=box['x']+box['width']/2;y=box['y']+box['height']/2
 page.evaluate('terrain3d.updateCompass()');bearing=page.locator('#first-person-compass').get_attribute('data-yaw')
 ...
 page.evaluate('terrain3d.updateCompass()');assert page.locator('#first-person-compass').get_attribute('data-yaw')!=bearing
 ...
 # The compass stays on screen in both views; only the walking minimap hides.
 assert not page.locator('#first-person-map').is_visible() and page.locator('#first-person-compass').is_visible()
 ...
 assert page.evaluate('terrain3d.firstPerson.view===0&&!terrain3d.firstPerson.walker.group.visible')
 assert page.evaluate('terrain3d.camera.position.distanceTo(terrain3d.firstPerson.eye)<1e-6')
 for _ in range(8):page.mouse.wheel(0,120)
