...
EXPECTED={
 ...
 'jeonuigam':('전의감','yukjo-compound'),
 'chunghunbu':('충훈부','yukjo-compound'),
 'jungbu':('한성부 중부','yukjo-compound'),
 'nambu':('한성부 남부','yukjo-compound'),
 'sajaegam':('사재감','yukjo-compound'),
 'naejasi':('내자시','yukjo-compound'),
 'manrichang':('만리창','yukjo-compound'),
 'gunjagam':('군자감 강감','yukjo-compound'),
}
with sync_playwright() as p:
 ...
 # Yuksanggung sits on its drawn 毓祥宮 label, north-west of the Gyeongbokgung wall.
 shrine=page.evaluate('''()=>{const t=terrain3d,f=t.buildings.children.find(b=>b.userData.feature.id==='yuksanggung').userData.feature;
 ...
 # Gwansanggam keeps its stone observation platform and sits west of Donhwamun.
 obs=page.evaluate('''()=>{const t=terrain3d,at=id=>t.buildings.children.find(b=>b.userData.feature.id===id);
  const o=at('gwansanggam'),m=o.getObjectByName('observatory').userData,g=at('donhwamun').position;
 ...
 # Pyeongsiseo sits on its drawn 平市署 label just south of the pagoda, clear of it.
 market=page.evaluate('''()=>{const t=terrain3d,at=id=>t.buildings.children.find(b=>b.userData.feature.id===id);
  const m=at('pyeongsiseo'),pg=at('wongaksa_pagoda'),[mw,,md]=m.userData.feature.symbol_size_m,[pw,,pd]=pg.userData.feature.symbol_size_m;
  return {pixel:m.userData.feature.source_position.pixel,southOfPagoda:m.position.z-pg.position.z,gapM:(m.position.z-md/2)-(pg.position.z+pd/2)}}''')
 print(market,flush=True)
 assert market['pixel']==[1593,1419] and market['southOfPagoda']>15 and market['gapM']>0,market
 # Yeonghuijeon sits on its drawn walled compound, well south of Hyeminseo.
 jinjeon=page.evaluate('''()=>{const t=terrain3d,at=id=>t.buildings.children.find(b=>b.userData.feature.id===id);
  const y=at('yeonghuijeon'),h=at('hyeminseo');return {pixel:y.userData.feature.source_position.pixel,detail:y.userData.feature.source_position.detail_asset,southOfHyeminseo:y.position.z-h.position.z}}''')
 print(jinjeon,flush=True)
 assert jinjeon['pixel']==[1643,1878] and jinjeon['detail']=='asset-0013' and jinjeon['southOfHyeminseo']>300,jinjeon
