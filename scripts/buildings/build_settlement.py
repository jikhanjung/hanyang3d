...
# The first 종묘 box reached south past Unjongga and blanked ordinary neighbourhoods; the drawn wall ends
# at y≈1345. Only the freed strip is sampled, from its own random stream, so every earlier house keeps its place.
precincts=[p if p['name']!='종묘' else {'name':'종묘','polygon':[[1795,1060],[1925,1060],[1925,1350],[1795,1350]]} for p in precincts]
freed=Image.new('L',size);fd=ImageDraw.Draw(freed);fd.polygon([tuple(p) for p in wall['centerline']],fill=255)
for p in precincts:
 fd.polygon([tuple(q) for q in p['polygon']],fill=0);fd.line([tuple(q) for q in p['polygon']+[p['polygon'][0]]],fill=0,width=17)
fd.line([tuple(p) for p in wall['centerline']],fill=0,width=35)
for o in wall['openings']:
 x,y=o['pixel'];fd.ellipse((x-24,y-24,x+24,y+24),fill=0)
for i,(a,b) in enumerate(zip(water['centerline'],water['centerline'][1:])):
 fd.line([tuple(a),tuple(b)],fill=0,width=int(2*max(water['half_widths_px'][i:i+2])+38))
freed_valid=(np.asarray(freed)>0)&near&clear&~(np.asarray(allowed)>0)
rng2=random.Random(17502);added=[]
for y in range(700,2260,3):
 for x in range(650,2570,3):
  px=x+rng2.uniform(-.6,.6);py=y+rng2.uniform(-.6,.6)
  if not freed_valid[round(py),round(px)]:continue
  ys,xs=np.nonzero(ink[y-34:y+35,x-34:x+35])
  if not len(xs):continue
  xs=xs+x-34;ys=ys+y-34;dist=(xs-px)**2+(ys-py)**2;k=int(np.argmin(dist));rx,ry=int(xs[k]),int(ys[k]);local=(xs-rx)**2+(ys-ry)**2<13**2
  xy=np.array([xs[local],ys[local]],float)
  ...
  vals,vec=np.linalg.eigh(np.cov(xy));v=vec[:,-1];angle=math.atan2(v[1],v[0])
  commercial=1100<px<2460 and 1370<py<1515 and dist[k]<18**2
  shop=commercial and rng2.random()<.72
  added.append([round(px,2),round(py,2),round(angle,4),round(rng2.uniform(5,8) if shop else rng2.uniform(4.5,7.5),2),round(rng2.uniform(4,6),2),round(rng2.uniform(2.6,3.5),2),int(shop),round(rng2.random(),5),rx,ry])
rng2.shuffle(added);features+=added
print('Freed-strip candidates',len(added))
