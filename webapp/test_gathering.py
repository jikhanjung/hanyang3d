...
from .models import Player,PlayerItem,FishingCast,HerbHarvest,Item,Trade
...
class GatheringTests(TestCase):
    ...
    def test_cast_wait_repeat_request_and_reward_only_once(self):
        ...
        self.assertEqual(a['token'],b['token']);self.assertEqual(a['wait_ms'],10000);self.assertEqual(a['duration_ms'],10000)
        self.assertEqual(self.post('/api/fishing',action='finish',token=a['token']).status_code,409)
        self.ready()
        with patch('webapp.fishing.secrets.randbelow',return_value=29):
            for _ in range(3):self.assertTrue(self.post('/api/fishing',action='finish',token=a['token']).json()['caught'])
        self.assertEqual(PlayerItem.objects.get(player=self.player,item='fish_pale_chub').quantity,1)
    ...
    def test_failure_cancellation_expiry_and_missing_rod(self):
        ...
        with patch('webapp.fishing.secrets.randbelow',return_value=30):self.assertFalse(self.post('/api/fishing',action='finish',token=a['token']).json()['caught'])
    ...
    def test_herb_probability_cooldown_and_account_scoping(self):
        ...
        self.assertEqual(PlayerItem.objects.get(player=self.player,item=catalog()['herbs']['nodes'][0]['item']).quantity,1)
    ...
    def test_species_rewards_are_durable_and_all_sell_at_distinct_prices(self):
        self.rod()
        for roll,expected in [(0,'fish_pale_chub'),(40,'fish_loach'),(65,'fish_crucian'),(90,'fish_carp')]:
            cast=self.post('/api/fishing',action='start',era=1907).json();self.ready()
            with patch('webapp.fishing.secrets.randbelow',side_effect=[0,roll]):
                answer=self.post('/api/fishing',action='finish',token=cast['token']).json()
            self.assertEqual(answer['item'],expected)
            repeated=self.post('/api/fishing',action='finish',token=cast['token']).json()
            self.assertEqual(repeated['item'],expected);self.assertEqual(PlayerItem.objects.get(player=self.player,item=expected).quantity,1)
        herbs=catalog()['herbs']
        for item in herbs['types']:
            node=next(n['id'] for n in herbs['nodes'] if n['item']==item)
            with patch('webapp.gathering.secrets.randbelow',return_value=0):
                answer=self.post('/api/herbs',action='gather',node=node).json()
            self.assertEqual(answer['item'],item);self.assertEqual(answer['cooldown_ms'],300000)
            self.assertEqual(PlayerItem.objects.get(player=self.player,item=item).quantity,1)
        for group,shop in [(list(herbs['types']),'약재상'),([r['item'] for r in catalog()['fishing']['catches']],'내어물전')]:
            self.assertEqual(len(set(sell_price(k) for k in group)),4)
            for item in group:
                Trade.objects.filter(player=self.player).update(created_at=timezone.now()-timedelta(seconds=2))
                before=Player.objects.get(pk=self.player.pk).money
                answer=self.post('/api/shop/trade',action='sell',shop=shop,item=item,quantity=1)
                self.assertEqual(answer.status_code,200);self.assertEqual(answer.json()['money'],before+sell_price(item))
