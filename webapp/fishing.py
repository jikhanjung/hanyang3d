    ...
    with transaction.atomic():
        ...
        if action=='start':
            ...
            if not cast or cast.status!='waiting' or now>cast.expires_at:
                cast,_=FishingCast.objects.update_or_create(player=player,defaults={'token':uuid.uuid4(),'era':era,'ready_at':now+timedelta(seconds=MIN_WAIT+secrets.randbelow(MAX_WAIT-MIN_WAIT+1)),'expires_at':now+timedelta(minutes=3),'status':'waiting','caught':False})
            ...
            if cast.status=='waiting':
                if action=='cancel' or now>cast.expires_at:
                    ...
                    if cast.caught:
                        stock,created=PlayerItem.objects.get_or_create(player=player,item='river_fish',defaults={'quantity':1})
                        if not created:PlayerItem.objects.filter(pk=stock.pk).update(quantity=F('quantity')+1)
                    cast.status='finished';cast.save(update_fields=['caught','status'])
        return {'token':str(cast.token),'status':cast.status,'caught':cast.caught,'wait_ms':max(0,int((cast.ready_at-now).total_seconds()*1000)),
                'message':('물고기 한 마리를 잡았소! 상인에게 팔 수 있소.' if cast.caught else '입질 없이 줄을 거두었소. 다시 드리워 보시오.') if cast.status=='finished' else '낚싯줄을 거두었소.' if cast.status=='cancelled' else '찌를 바라보며 기다리는 중…'}
