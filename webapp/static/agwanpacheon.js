...
export function createAgwanpacheon({data,scene,camera,walking,buildings,infrastructure,settlement,trams,material,surface,channel}){
 ...
 async function begin(restart=false){
  ...
  try{
   ...
   saved=await api('start');checkpoint=saved.checkpoint;distance=Math.min(lengths.at(-1),Math.max(data.spacing_m+7,milestones[checkpoint]));saveError=false;finished=false;arrival?.reset();arrival=null;
 ...
 function update(dt){
  ...
  if(arrival){if(arrival.update(Math.min(dt,.1)))complete();return}
 ...
 return {update,begin,regroup,root,chairs,get arrival(){return arrival},guard,get neighborhood(){return neighborhood},get path(){return path},get milestones(){return milestones},get active(){return active},get distance(){return distance},get checkpoint(){return checkpoint},get saving(){return writeCount>0}};
}
