...
export function createGyeonghoeruPond(feature,w,h,d){
 const parts=[],y0=-h/2,rim=1.2,depth=.9;
 ...
 // Pavilion island: 7 by 5 bays, 34.4 by 28.5 m, on a stone platform ringed by a low kerb.
 const pw=34.4,pd=28.5,px=w/2-rim-8-pw/2-2,pz=-4;
 box(parts,'pavilion-platform','granite',px,y0+depth/2+.15,pz,pw+4,depth+.3,pd+4);
 for(let i=0;i<8;i++)for(let j=0;j<6;j++){
  const outer=i===0||i===7||j===0||j===5,x=px-pw/2+i*pw/7,z=pz-pd/2+j*pd/5;
  const pillar=new THREE.Mesh(outer?new THREE.BoxGeometry(.8,4.6,.8):new THREE.CylinderGeometry(.32,.36,4.6,10),mats.darkStone);
  pillar.position.set(x,y0+depth+.3+2.3,z);pillar.name='stone-pillar';parts.push(pillar);
 }
 for(const z of [-9,0,9])box(parts,'stone-bridge','granite',w/2-rim-4-1,y0+depth+.1,pz+z,8,.5,2.6);
 // Two square islands with earth and grass, on the west side.
 for(const [ix,iz] of [[-w/2+rim+22,-d/2+rim+30],[-w/2+rim+22,d/2-rim-30]]){
  box(parts,'island-kerb','granite',ix,y0+depth/2+.1,iz,13,depth+.2,13);box(parts,'island-top','grass',ix,y0+depth+.25,iz,11.5,.3,11.5);
 }
...
// Geunjeongjeon site: the two-tier terrace (월대, 2.72 m in all) with south stairs and the column footings of the
// 5-by-5-bay hall (30 by 21 m) on top; the hall itself did not exist in 1750.
export function createGeunjeongjeonSite(feature,w,h,d){
 const parts=[],y0=-h/2,lowH=1.3,upH=1.42,hallW=30,hallD=21;
 box(parts,'lower-terrace','granite',0,y0+lowH/2,0,w,lowH,d);
 box(parts,'upper-terrace','granite',0,y0+lowH+upH/2,-2,w*.8,upH,d*.78);
 ...
 stairs(y0,lowH,d/2,12);stairs(y0+lowH,upH,d*.78/2-2,10);
 // Column footings: 6 by 6 stones for the 5-by-5 bays, with the inner ring for the high columns.
 for(let i=0;i<6;i++)for(let j=0;j<6;j++)box(parts,'column-footing','darkStone',-hallW/2+i*hallW/5,y0+lowH+upH+.25,-2-hallD/2+j*hallD/5,.9,.5,.9);
 box(parts,'hall-floor','earth',0,y0+lowH+upH+.06,-2,hallW+1,.12,hallD+1);
 const model=merge(parts,'geunjeongjeon-site');
 model.userData={conceptual:true,terraceM:[w,d],hallM:[hallW,hallD],footings:36,period:feature.temporal};
 return model;
}
