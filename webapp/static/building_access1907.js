...
export function addBuildingAccess1907(model,groundAt){
 ...
 const yaw=model.userData.accessYaw??0,offset=model.userData.accessOffset??[0,0],axis=new THREE.Vector3(0,1,0);
 const vertices=[],point=new THREE.Vector3(),v=new THREE.Vector3();
 let end=front;
 model.updateMatrixWorld(true);
 for(let i=0;i<256;i++){
  const z=front+(i+.5)*tread,top=floor+entryHeight-(i+1)*riser;
  const heights=[-width/2,0,width/2].map(dx=>{point.set(x+dx,-h/2,z+tread/2).applyAxisAngle(axis,yaw);point.x+=offset[0];point.z+=offset[1];model.localToWorld(point);return groundAt(point.x,point.z)});
  ...
  geometry.rotateY(yaw);geometry.translate(offset[0],0,offset[1]);
 ...
 model.userData.access={x,front,end,width,riser,tread,yaw,offset,estimated:true};
}
