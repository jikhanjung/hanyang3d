...
export function createLegationGuard({scene,building,groundAt,infrastructure}){
 ...
 const [width,height,depth]=building.userData.feature.symbol_size_m,world=(x,z)=>building.localToWorld(new THREE.Vector3(x,-height/2,z));
 // Inside the footprint the model's floor is the raised plinth top; only beyond its edge does the terrain apply.
 const standing=(x,z)=>{const p=world(x,z);if(Math.abs(x)>=width/2||Math.abs(z)>=depth/2)p.y=groundAt(p.x,p.z);return p};
 const front=depth*.275,positions=[];
 ...
 for(const [x,z] of [[-4,12],[4,12],[-12,20],[12,20]]){
  ...
  p.group.position.copy(standing(x,z));p.group.rotation.y=building.rotation.y;group.add(p.group);guards.push(p);
 }
 return {group,guards,world,standing};
}
export function createRoyalArrival({scene,camera,container,chairs,building,fp,say,guard}){
 ...
 const {world,standing}=guard,door=world(0,building.userData.feature.symbol_size_m[2]*.275),inside=world(0,building.userData.feature.symbol_size_m[2]*.275-3);
 const actors=chairs.map((c,i)=>{
  ...
  const approach=standing(i?-.5:.5,15),points=[start,out,approach,door.clone(),inside.clone()];
  p.group.visible=false;group.add(p.group);return {p,points,delay:i*2};
 });
 const host=createWalker();host.group.name='legation-interpreter';host.group.position.copy(standing(-2.5,13));host.group.rotation.y=building.rotation.y;group.add(host.group);
 ...
 function update(dt){
  ...
  for(const [i,{p,points,delay}] of actors.entries()){
   ...
   p.group.position.copy(a).lerp(b,u);if(t>=3&&t<16)p.group.position.y=Math.max(b.y,fp.groundAt(p.group.position.x,p.group.position.z)??b.y);
