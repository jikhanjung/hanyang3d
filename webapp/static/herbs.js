...
export function createHerbs({scene,camera,canvas,firstPerson,shop,dialogue,data,source,groundAt,collision,market,era,visible=()=>true}){
 ...
 const mesh=new THREE.InstancedMesh(geometry,new THREE.MeshStandardMaterial({color:0xffffff,roughness:1,side:THREE.DoubleSide}),rows.length);mesh.name='gatherable-herbs';scene.add(mesh);
 const flowers=new THREE.InstancedMesh(new THREE.SphereGeometry(1,10,5),new THREE.MeshStandardMaterial({color:0xffffff,roughness:1}),rows.length),stems=new THREE.InstancedMesh(new THREE.CylinderGeometry(.025,.035,1,5),new THREE.MeshStandardMaterial({color:0x638648}),rows.length);flowers.name='herb-flowers';stems.name='herb-stems';scene.add(flowers,stems);
 const typeFor=r=>data.herbs.types?.[r.item]??{color:'#76a843',scale:[1,1,1]};
 rows.forEach((r,i)=>{const type=typeFor(r);mesh.setColorAt(i,new THREE.Color(type.color));flowers.setColorAt(i,new THREE.Color(type.bloom??'#ffffff'))});
 ...
 function refreshPlants(){const now=performance.now();rows.forEach((r,i)=>{
  const shown=(cooldowns.get(r.id)??0)<=now,type=typeFor(r);if(shown)cooldowns.delete(r.id);
  dummy.position.copy(r.position);dummy.rotation.set(0,i*2.4,0);dummy.scale.fromArray(type.scale).multiplyScalar(shown?1:0);dummy.updateMatrix();mesh.setMatrixAt(i,dummy.matrix);
  const height=r.item==='herb_bellflower'?1.05:r.item==='herb_plantain'?.65:.5;
 ...
 return {mesh,flowers,stems,rows,update,merchant:g,merchantAt,cooldowns};
}
