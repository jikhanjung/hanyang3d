...
한양3D는 Django·Three.js 기반의 지도 서비스다. 현재 DB, 계정, 사용자 업로드는 없다. 선택 기능인 '함께 걷기'는 별도 Colyseus 프로세스에서 접속자의 이름·위치와 NPC 이동을 메모리에 공유한다. 이름과 마지막 걷기 위치·방향은 브라우저 localStorage에 기억한다. 실행·범위는 `multiplayer/README.md`를 참고한다.
