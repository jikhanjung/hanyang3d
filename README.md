...
현재는 **자료 확보와 지도 검토 도구를 갖추고 좌표 정합을 검증하는 단계**입니다(2026-09-09 기준). 현재 서버에 도성대지도 JPG 2건, 1908 지적도 JPG 4건·영인 도록 PDF 1건, 1912 지적도 JPG 5건(뷰어 표시용 1건 포함)을 확보하고 메타데이터를 등록했습니다. 검증된 지리 좌표를 가진 GIS 데이터, 3D 모델과 3D 뷰어는 아직 없습니다. 도성 전도를 현재 지형에 겹치는 검토용 TPS 화면은 제공합니다.
...
도성대지도 중심으로 한양 도성 전체의 모습을 먼저 보여 줍니다. 고도 데이터로 만든 3D 지형에 도성대지도를 입혀 산세와 주요 길·하천의 관계를 둘러보는 것이 다음 시각화 목표입니다. 기존 평면 TPS 배치를 활용하고 성문·성곽·가로·수계를 순차 판독합니다. 건물 복원은 별도 근거를 확보한 뒤 진행하며, 1908·1912 세부 도엽 정합은 후속 참고 작업으로 보존합니다. [우선순위 변경 결정](docs/decisions/0002-citywide-map-first.md)을 참고하세요.
