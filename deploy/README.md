...
이미지: **`honestjung/hanyang3d:v0.1.22`**, 플랫폼: `linux/amd64`.
...
`v0.1.22`는 전체 화면 3D에 옛지도 슬라이더(50%), 보정점(기본 꺼짐), 1인칭 걷기 버튼을 제공한다. 청계천은 원도 구간만 표시한다. 이전 `v0.0.1` 이미지와 데이터는 서버에 보관해 롤백할 수 있다.
...
```text
dist/hanyang3d-image-v0.1.22.tar.gz
dist/hanyang3d-data-v0.1.22.tar.gz
dist/hanyang3d-host-v0.1.22.tar.gz
dist/SHA256SUMS-v0.1.22
```
...
scp dist/hanyang3d-*-v0.1.22.tar.gz dist/SHA256SUMS-v0.1.22 dolfinid:~/hanyang3d-release/
```
...
sha256sum -c SHA256SUMS-v0.1.22
docker load -i hanyang3d-image-v0.1.22.tar.gz
sudo install -d -o "$(id -un)" -g "$(id -gn)" /srv/hanyang3d
tar -xzf hanyang3d-host-v0.1.22.tar.gz -C /srv/hanyang3d
mkdir -p /srv/hanyang3d/data
mkdir /srv/hanyang3d/data/v0.1.22
tar -xzf hanyang3d-data-v0.1.22.tar.gz -C /srv/hanyang3d/data/v0.1.22
cd /srv/hanyang3d
