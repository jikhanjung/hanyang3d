...
이미지: **`honestjung/hanyang3d:v0.1.0`**, 플랫폼: `linux/amd64`.
...
- Docker Hub: `honestjung/hanyang3d:v0.1.0`
- 배포 digest: `sha256:9d99525715e074a5876d46e8fecd8f0c7224715195acfdb5daea3758cbaa8fa0`
...
`v0.1.0`는 전체 화면 3D에 옛지도 슬라이더(50%), 보정점(기본 꺼짐), 1인칭 걷기 버튼을 제공한다. 청계천은 원도 구간만 표시한다. 이전 `v0.0.1` 이미지와 데이터는 서버에 보관해 롤백할 수 있다.
...
```text
dist/hanyang3d-image-v0.1.0.tar.gz
dist/hanyang3d-data-v0.1.0.tar.gz
dist/hanyang3d-host-v0.1.0.tar.gz
dist/SHA256SUMS-v0.1.0
```
...
scp dist/hanyang3d-*-v0.1.0.tar.gz dist/SHA256SUMS-v0.1.0 dolfinid:~/hanyang3d-release/
```
...
sha256sum -c SHA256SUMS-v0.1.0
docker load -i hanyang3d-image-v0.1.0.tar.gz
sudo install -d -o "$(id -un)" -g "$(id -gn)" /srv/hanyang3d
tar -xzf hanyang3d-host-v0.1.0.tar.gz -C /srv/hanyang3d
mkdir -p /srv/hanyang3d/data
mkdir /srv/hanyang3d/data/v0.1.0
tar -xzf hanyang3d-data-v0.1.0.tar.gz -C /srv/hanyang3d/data/v0.1.0
cd /srv/hanyang3d
...
v0.1.0에서는 지면·지도·도로가 같은 고도 표면을 사용한다. 지도 0%에서도 배치가 유지되며 눈높이는 지면 위 1.65m다.

v0.1.0은 경복궁 담장·종묘 정전 15칸 모형, 한양3D 제목·버전 표시와 현재 지도 중심의 출처 페이지를 추가한다.

v0.1.0은 모바일 조이스틱·동시 시선 드래그, 보정점 체크박스 숨김, 상시 우상단 나침반과 반투명 미니맵을 제공한다.

v0.1.0에서는 모바일 조작부를 반투명 조이스틱만 남기고 간소화했다.

v0.1.0은 육조거리 관청 11곳과 건물별 연대 메타데이터를 추가하고, 모바일 상단 옵션을 설정 버튼으로 접는다.

v0.1.0은 PC·모바일 공통으로 수평 3D 나침반을 제공한다. 모바일 좌상단에는 제목·버전과 햄버거만 표시하며 걷기·지도 옵션은 메뉴로 연다.

v0.1.0에서는 기존 초기화면 링크를 조작부에서 제거했다.
...
v0.1.0은 북촌·서촌 지역 이름과 반투명 제목/출처 배경, 모바일 왼쪽 햄버거 및 © 출처 링크를 제공한다.

마우스: 좌클릭 드래그 이동, 휠 버튼 드래그 회전, 스크롤/우클릭 드래그 확대·축소.

v0.1.0는 서촌을 경복궁 서쪽 담장과 상류 사이로, 북촌을 기존 표기와 안국방 글씨 사이로 옮긴다. 두 이름은 원도 기준 위치를 사용한다.

v0.1.0는 사직단·경덕궁·문묘·성균관을 원도에 그려진 건물과 담장 내부로 맞춘다.

v0.1.0는 기본 지면을 황토색, 수목 주변을 녹지로 표현한다. 북악산·인왕산 화강암 중심부의 수목 후보 유지 확률은 25%이며 가장자리에서는 정상 밀도로 이어진다.

v0.1.0은 경덕궁·창덕궁·창경궁을 상자 대신 단순화한 궁궐 모형으로 표시한다.

v0.1.0은 황토색을 밝고 연하게 바꾸고 모든 지도 이름의 글자 크기·굵기를 통일한다.

최종 v0.1.0는 창덕궁을 원도의 전각 지붕 자리로 옮기고 돈화문·홍화문 모형을 추가한다. 창경궁과 홍화문은 두 모형의 중간이 기존 위치와 맞도록 배치한다.

v0.1.0은 걷기 시점을 3인칭으로 바꿔 내 캐릭터를 보여 주고, 걷는 속도를 3 m/s로 올린다. 휠로 시점 거리를 조절하며 0이면 기존 1인칭 화면이다.

v0.1.0는 정도전·송시열 집을 담장·기와집 모형으로 표시하고 설정의 `유명인 집터`로 껐다 켤 수 있게 한다.

v0.1.0은 두 집터 모형을 남향으로 세우고 정도전 집을 북동쪽으로 20 m 옮긴다.

v0.1.0는 종루·의금부·좌우 포도청·훈련원·경모궁을 추가하고, 종루와 의금부를 원도의 길에 맞춰 사거리 밖 블록에 둔다.

v0.1.0은 종로1가 사거리를 보정점으로 추가하고 그에 맞춰 현대 좌표 기준 배치를 다시 계산한다. v0.0.25 이미지는 배포하지 않았다.

v0.1.0은 훈련원과 좌포도청을 원도의 길 밖으로 옮긴다.

v0.1.0은 종루를 보신각 형태의 중층 누각으로 다시 만든다.

v0.1.0는 운종가 시전 행랑과 넓힌 훈련원을 추가하고 표시 전환을 설정 패널로 묶는다.

v0.1.0은 시전 구간을 창덕궁 동구까지로 줄이고 대로를 넓히며 가게 앞을 연 모습으로 바꾼다.

v0.1.0은 시전 좌판 뒤 상자를 없애고 가게 주인과 시전 이름 간판을 넣는다.
