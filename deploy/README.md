...
이미지: **`honestjung/hanyang3d:v0.1.13`**, 플랫폼: `linux/amd64`.
...
- Docker Hub: `honestjung/hanyang3d:v0.1.13`
- 배포 digest: `sha256:e1c61b3ac3730011f3b1c63070ce6705f24621b6422e00db0860b30319a558e6`
...
`v0.1.13`는 전체 화면 3D에 옛지도 슬라이더(50%), 보정점(기본 꺼짐), 1인칭 걷기 버튼을 제공한다. 청계천은 원도 구간만 표시한다. 이전 `v0.0.1` 이미지와 데이터는 서버에 보관해 롤백할 수 있다.

## 구성과 데이터

- 이미지: Django/Three.js 코드, 카탈로그, GIS 판독·배치 JSON, 배포 도구.
- 데이터 묶음: 카탈로그 원본 23개와 웹에서 제공하는 파생 산출물 8개. 경로·크기·SHA-256을 `manifest.json`에 기록한다.
- 컨테이너: Gunicorn, UID/GID `10001`, 읽기 전용 루트와 `/runtime`, 임시 작업용 `/tmp`.
- `/healthz`: 버전 및 제공 파일 78개의 존재 상태. 데이터 누락·버전 불일치 시 503.
...
```text
dist/hanyang3d-image-v0.1.13.tar.gz
dist/hanyang3d-data-v0.1.13.tar.gz
dist/hanyang3d-host-v0.1.13.tar.gz
dist/SHA256SUMS-v0.1.13
```
...
scp dist/hanyang3d-*-v0.1.13.tar.gz dist/SHA256SUMS-v0.1.13 dolfinid:~/hanyang3d-release/
```
...
sha256sum -c SHA256SUMS-v0.1.13
docker load -i hanyang3d-image-v0.1.13.tar.gz
sudo install -d -o "$(id -un)" -g "$(id -gn)" /srv/hanyang3d
tar -xzf hanyang3d-host-v0.1.13.tar.gz -C /srv/hanyang3d
mkdir -p /srv/hanyang3d/data
mkdir /srv/hanyang3d/data/v0.1.13
tar -xzf hanyang3d-data-v0.1.13.tar.gz -C /srv/hanyang3d/data/v0.1.13
cd /srv/hanyang3d
