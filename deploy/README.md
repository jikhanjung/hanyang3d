...
이미지: **`honestjung/hanyang3d:v0.0.10`**, 플랫폼: `linux/amd64`.
...
- Docker Hub: `honestjung/hanyang3d:v0.0.10`
- 배포 digest: `sha256:95bcbfe81c7f2f978fa3bf124bdf33fffe5e3d39f75c03ab1a4daea2bb374ba4`
...
`v0.0.10`는 전체 화면 3D에 옛지도 슬라이더(50%), 보정점(기본 꺼짐), 1인칭 걷기 버튼과 기존 초기화면 링크를 제공한다. 청계천은 원도 구간만 표시한다. 이전 `v0.0.1` 이미지와 데이터는 서버에 보관해 롤백할 수 있다.
...
- `/healthz`: 버전 및 제공 파일 55개의 존재 상태. 데이터 누락·버전 불일치 시 503.
...
```text
dist/hanyang3d-image-v0.0.10.tar.gz
dist/hanyang3d-data-v0.0.10.tar.gz
dist/hanyang3d-host-v0.0.10.tar.gz
dist/SHA256SUMS-v0.0.10
```
...
scp dist/hanyang3d-*-v0.0.10.tar.gz dist/SHA256SUMS-v0.0.10 dolfinid:~/hanyang3d-release/
```
...
sha256sum -c SHA256SUMS-v0.0.10
docker load -i hanyang3d-image-v0.0.10.tar.gz
sudo install -d -o "$(id -un)" -g "$(id -gn)" /srv/hanyang3d
tar -xzf hanyang3d-host-v0.0.10.tar.gz -C /srv/hanyang3d
mkdir -p /srv/hanyang3d/data
mkdir /srv/hanyang3d/data/v0.0.10
tar -xzf hanyang3d-data-v0.0.10.tar.gz -C /srv/hanyang3d/data/v0.0.10
cd /srv/hanyang3d
...
v0.0.10에서는 지면·지도·도로가 같은 고도 표면을 사용한다. 지도 0%에서도 배치가 유지되며 눈높이는 지면 위 1.65m다.

v0.0.10은 경복궁 담장·종묘 정전 15칸 모형, 한양3D 제목·버전 표시와 현재 지도 중심의 출처 페이지를 추가한다.

v0.0.10은 모바일 조이스틱·동시 시선 드래그, 보정점 체크박스 숨김, 상시 우상단 나침반과 반투명 미니맵을 제공한다.

v0.0.10에서는 모바일 조작부를 반투명 조이스틱만 남기고 간소화했다.
