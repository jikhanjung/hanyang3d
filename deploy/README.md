...
이미지: **`honestjung/hanyang3d:v0.1.35`**, 플랫폼: `linux/amd64`.
...
- Docker Hub: `honestjung/hanyang3d:v0.1.35`
...
`v0.1.35`는 전체 화면 3D에 옛지도 슬라이더(50%), 보정점(기본 꺼짐), 1인칭 걷기 버튼을 제공한다. 청계천은 원도 구간만 표시한다. 서버에는 운영 버전과 직전 버전의 이미지·데이터만 보관한다(`prune.sh`).
...
```text
dist/hanyang3d-image-v0.1.35.tar.gz
dist/hanyang3d-data-v0.1.35.tar.gz
dist/hanyang3d-host-v0.1.35.tar.gz
dist/SHA256SUMS-v0.1.35
```
...
scp dist/hanyang3d-*-v0.1.35.tar.gz dist/SHA256SUMS-v0.1.35 dolfinid:~/hanyang3d-release/
```
...
sha256sum -c SHA256SUMS-v0.1.35
docker load -i hanyang3d-image-v0.1.35.tar.gz
sudo install -d -o "$(id -un)" -g "$(id -gn)" /srv/hanyang3d
tar -xzf hanyang3d-host-v0.1.35.tar.gz -C /srv/hanyang3d
mkdir -p /srv/hanyang3d/data
mkdir /srv/hanyang3d/data/v0.1.35
tar -xzf hanyang3d-data-v0.1.35.tar.gz -C /srv/hanyang3d/data/v0.1.35
cd /srv/hanyang3d
