...
web_only=${2:-}
[[ -z "$web_only" || "$web_only" == --web-only ]] || exit 1
image="honestjung/hanyang3d:$version"
multiplayer_image="honestjung/hanyang3d-multiplayer:$version"
...
"$python_bin" deploy/smoke_content.py "$image" ".build/runtime-$version"
docker save "$image" | gzip > "dist/hanyang3d-image-$version.tar.gz"
if [[ "$web_only" != --web-only ]]; then
    docker build --platform linux/amd64 -f deploy/Dockerfile.multiplayer \
        --build-arg "APP_VERSION=$version" --build-arg "VCS_REF=$revision" -t "$multiplayer_image" .
    "$python_bin" deploy/smoke_multiplayer.py "$multiplayer_image"
    docker save "$multiplayer_image" | gzip > "dist/hanyang3d-multiplayer-image-$version.tar.gz"
fi
...
(
    ...
    if [[ "$web_only" != --web-only ]]; then
        sha256sum "hanyang3d-multiplayer-image-$version.tar.gz" >> "SHA256SUMS-$version"
    fi
)
