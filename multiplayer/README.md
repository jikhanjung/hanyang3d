...
두 브라우저에서 `http://127.0.0.1:18014`에 접속해 설정 메뉴의 **1인칭**를 누른다. 처음 걷기에 들어갈 때 1~16자의 이름을 입력한다. 이름은 브라우저의 localStorage에 기억하며 다른 접속자에게 머리 위 이름표로 표시한다. 같은 공간에서 이미 접속 중인 이름은 사용할 수 없다. 공백 정리·유니코드 NFC 정규화 후 영문 대소문자도 구분하지 않는다. 중복이면 이름 입력창에서 바꿔 다시 접속한다. 취소하면 걷기에 들어가지 않는다.

같은 위치에서 시작하므로 한 사람이 몇 걸음 움직이면 서로를 볼 수 있다. 서버가 공간 내 접속자마다 서로 다른 옷 색깔을 배정하고 자신의 화면과 다른 접속자의 화면에 같은 색을 표시한다. 퇴장하면 이름과 색깔은 다시 사용할 수 있다. 전체 지도 시점으로 돌아가거나 ‘전체 지도 시점’을 누르면 접속도 끝난다. 연결 실패 후에는 버튼으로 다시 접속한다.
...
- 로그인, 서버 영구 저장, 음성 채팅, 접속자끼리의 충돌은 아직 없다. 서버의 접속자 정보는 퇴장/서버 종료 시 사라진다. 이름만 각 브라우저의 localStorage에 기억한다.
