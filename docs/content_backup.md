...
1. **배포 전**: 새 migration 전에 즉시 검증된 스냅샷을 별도 `pre-deploy` 경로에 둔다. 시간별 prune은 이 경로를 건드리지 않는다. 코드·데이터 버전과 함께 기록한다. 새 스냅샷을 검증·채택한 뒤 같은 폴더의 배포 전 스냅샷을 최신 10개만 남긴다(`--keep-snapshots`).
