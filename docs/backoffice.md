...
로컬 기본 모드는 SQLite DB이며 관리 화면은 `/backoffice/`다. 최초 가져오기로 건물 103곳, 이야기 33편, 상세 설명 102개, 리소스 108개(모형 생성기 15개 + 공개 파일 93개)를 등록한다.
...
가져온 뒤부터 DB가 콘텐츠 정본이다. Git의 JSON·Markdown을 수정해도 운영 DB에 자동 덮어쓰지 않는다. 출처·좌표·표시 순서와 기존 안내 문단을 초기 자료와 비교하는 테스트를 둔다. 지도 모형의 원도 좌표·치수·고증 상태는 건물의 고급 JSON 설정에 그대로 보존한다.
...
현재 운영 v0.2.4는 파일 모드다. 이번 프레임워크를 개발했다고 운영 DB를 자동 생성하거나 기존 서비스를 전환하지 않는다. Docker 이미지 기본값도 기존 배포 호환을 위해 `files`다.
...
이후 DB 스키마를 바꾸는 릴리스는 **검증된 배포 전 백업 → 웹 중지 → 일회용 컨테이너 migrate → 웹 시작 → 확인** 순서다. 시작 스크립트는 미적용 migration을 검사하고 거부하며 자동 migration·재가져오기를 하지 않는다. 기존 `deploy.sh`는 아직 DB migration·백업을 자동 수행하지 않으므로 DB 전환 후 이 순서를 생략한 채 단독 실행하지 않는다.

[백업과 복원](content_backup.md)을 먼저 구성한다. 자동 운영 전환·정기 작업 등록·오프사이트/NAS 연결은 별도 설치 작업이다.
